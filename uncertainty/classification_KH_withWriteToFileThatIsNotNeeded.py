...
This file is modified from Johannes' version by Kristian Dalsbø Hindberg
so that it save all the pdf values used in the classifier and also calculates
and save the Mahalnobis distance of each class for each pixel.

Kristian Dalbsø Hindberg @ CIRFA, January 2023.
""" 
import os
...
# -------------------------------------------------------------------------- #
  ...
  result_base_Mahal      = f_base + '_Mahal'
  result_path_Mahal      = result_folder / f'{result_base_Mahal}.img'
  result_path_Mahal_hdr  = result_folder / f'{result_base_Mahal}.hdr'
  result_base_probs      = f_base + '_probs'
  result_path_probs      = result_folder / f'{result_base_probs}.img'
  result_path_probs_hdr  = result_folder / f'{result_base_probs}.hdr'
  ...
  if result_path_Mahal.is_file() and overwrite:
    os.remove(result_path_Mahal.as_posix())
    os.remove(result_path_Mahal_hdr.as_posix())
  if result_path_probs.is_file() and overwrite:
    os.remove(result_path_probs.as_posix())
    os.remove(result_path_probs_hdr.as_posix())
# -------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------- #
  ...
  probs_out = np.copy(probs_img)
  Mahal_out = np.copy(Mahal_img)

  # reshape to image geometry
  labels_img = np.reshape(labels_img,shape)
  #probs_img  = np.reshape(probs_img,(shape[0], shape[1], n_classes) )
  #Mahal_img  = np.reshape(Mahal_img,(shape[0], shape[1], n_classes) )
# -------------------------------------------------------------------------- #
  ...
  # get drivers
  output_labels  = gdal.GetDriverByName('Envi').Create(result_path.as_posix()      , Nx, Ny, 1, gdal.GDT_Byte) # GDT_Byte
  ...
  # write labels of class 1 to band 1 and
  # the Mahalnabois distance and probabilities of each class to different bands
  output_labels.GetRasterBand(1).WriteArray(labels_img)
  #for classy in range(n_classes):
  #    bandy = classy + 1
  #    output_Mahal.GetRasterBand(bandy).WriteArray(Mahal_img[:,:,classy])
  #    output_probs.GetRasterBand(bandy).WriteArray(probs_img[:,:,classy])

  # flush
  output_labels.FlushCache()
  #output_Mahal.FlushCache()
  #output_probs.FlushCache()
